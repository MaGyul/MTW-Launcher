...
export class AnsiUp
{
    ...
    constructor() {
        ...
        this.ansi_colors = this.palette_256 = this._csi_regex = this._osc_st  = this._osc_regex = this._boldStyle = this._faintStyle = this._italicStyle = this._underlineStyle = undefined as any;
        this.setup_palettes();
        ...
        this.fg = this.bg = undefined as any;
    ...
    private get_next_packet(): TextPacket | undefined {
    ...
    private process_ansi(pkt:TextPacket)
    {
      ...
      while (sgr_cmds.length > 0) {
          let sgr_cmd_str = sgr_cmds.shift() as string;
      ...
      // AT SOME POINT, JUST CONVERT TO A LOOKUP TABLE
          if (isNaN(num) || num === 0) {
          ...
          } else if (num === 39) { this.fg = null as any;
          } else if (num === 49) { this.bg = null as any;
              ...
              if (sgr_cmds.length > 0) {
                  ...
                  if (mode_cmd === '5' && sgr_cmds.length > 0) {
                      let palette_index = parseInt(sgr_cmds.shift() as string, 10);
                  ...
                  if (mode_cmd === '2' && sgr_cmds.length > 2) {
                      let r = parseInt(sgr_cmds.shift() as string, 10);
                      let g = parseInt(sgr_cmds.shift() as string, 10);
                      let b = parseInt(sgr_cmds.shift() as string, 10);
    ...
    private process_hyperlink(pkt:TextPacket):string
    {
        ...
        if (! (this._url_allowlist as any)[parts[0]])
            return '';
